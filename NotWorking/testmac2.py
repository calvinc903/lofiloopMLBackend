...
if device.type == 'mps':
    ...
    # Patch the condition provider to ensure its outputs are on the MPS device
    orig_condition_provider = model.lm.condition_provider
    def patched_condition_provider(tokens):
        cond = orig_condition_provider(tokens)
        # If the condition provider returns a tuple (conditions, mask), move both to the current device
        if isinstance(cond, tuple) and len(cond) == 2:
            return (cond[0].to(device), cond[1].to(device))
        # If it returns a single tensor, move it to device
        elif isinstance(cond, torch.Tensor):
            return cond.to(device)
        return cond
    model.lm.condition_provider = patched_condition_provider
